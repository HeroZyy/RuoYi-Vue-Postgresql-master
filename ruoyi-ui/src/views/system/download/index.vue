<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="是否允许下载" prop="allowDl">
        <el-input
          v-model="queryParams.allowDl"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:download:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:download:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:download:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:download:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="downloadList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="下载表自增主键" align="center" prop="sid" />
      <el-table-column label="产品id" align="center" prop="prodId" />
      <el-table-column label="下载地址" align="center" prop="url" />
      <el-table-column label="是否允许下载" align="center" prop="allowDl" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="是否允许下载" prop="allowDl">
          <el-input v-model="form.allowDl" placeholder="请输入是否允许下载" />
        </el-form-item>
      </el-form>
...
<script>
import { listDownload, getDownload, delDownload, addDownload, updateDownload } from "@/api/system/download";
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加产品资料下载信息";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const sid = row.sid || this.ids
      getDownload(sid).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改产品资料下载信息";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除产品资料下载信息编号为"' + sids + '"的数据项？').then(function() {
        return delDownload(sids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/download/export', {
        ...this.queryParams
